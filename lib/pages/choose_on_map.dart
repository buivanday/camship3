...
class _ChooseOnMapState extends State<ChooseOnMap> {
  ...
  Future _getAddress(lat, lng)async{
    var responseJson = await NetworkUtils.postWithBodyWithoutAuth('/api/Zones/find-address', {
      "lat": lat,
      "lng": lng
    });
    return responseJson;
  ...
  void _onMapCreated(GoogleMapController controller) async {
    ...
    if(Platform.isIOS) {
     ...
     try{
      var json =await _getAddress(location.latitude, location.longitude).then((_){
        _addressController.text= _.first["formattedAddress"];
         _lastMapPosition = new LatLng(location.latitude, location.longitude);
      ...
      _markers = {
        Marker(
          ...
          icon: BitmapDescriptor.fromBytes(markerIcon),//BitmapDescriptor.fromAsset('icons/current.png'),
        )
      };
     }catch( e){
  ...
  void _onCameraIdle() async {
    ...
    try {
     ...
     var json =await _getAddress(_lastMapPosition.latitude, _lastMapPosition.longitude).then((_){
        _addressController.text= _.first["formattedAddress"];  
      });
      setState(() {
        _isValid = true;
    ...
    showModalBottomSheet(
      ...
      builder: (BuildContext bc){
          return Container(
            ...
            child: Column(
              children: <Widget>[
                Text(allTranslations.text('list_zones'), style: TextStyle(color: HexColor('#455A64'), fontWeight: FontWeight.bold, fontSize: 18.0), ),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Container(
        child: Column(
          children: <Widget>[
            GradientAppBar(title: allTranslations.text('choose_on_map'),hasBackIcon: true),
            Expanded(
              ...
              child: Container(
                ...
                child: Column(
                  children: <Widget>[
                    Theme(
                      ...
                      child: TextField(
                        ...
                        onTap: () async {
                          ...
                          if(result!=null){
                          _addressController.text = result["result"]["formatted_address"];
                          final coordinates = new LatLng(result['result']['geometry']['location']['lat'],result['result']['geometry']['location']['lng']);
                          setState(() {
                          ...
                          _onCameraMove(new CameraPosition(target: coordinates));
                          mapController?.moveCamera(
                            CameraUpdate.newCameraPosition(
                              CameraPosition(
                                target: coordinates,
                    ...
                    RaisedButton(
                      onPressed: _isValid&&!isOffline&&isLoading ? () async {
                       setState(() {
                         isLoading = false; 
                       });
                        if(widget.isGetStarted) {
                          Navigator.pop(context, false);
                          Navigator.push(context, MaterialPageRoute(
                              builder: (context) => GetStarted(isReturnedFromChooseMap: true, oldData: widget.oldData, chosenAddress: _addressController.text, lat: _lastMapPosition.latitude, lng: _lastMapPosition.longitude)
                          ));
                        } else {
                          
                          List<dynamic> zones = await NetworkUtils.fetchWithoutAuthorization('/api/Zones');
                          var locationDetails = await NetworkUtils.httpGetDetailFromLatLng(_lastMapPosition.latitude.toString(), _lastMapPosition.longitude.toString());
                          if(locationDetails['status'] == 'OK') {
                            setState(() {
                              isLoading = true; 
                            });
                            List<dynamic> results = locationDetails['results'];
                            if(results.isNotEmpty) {
                              String longName = '';
                              String shortName = '';
                              String address = '';
                              for (var i = 0; i < results.length; i++) {
                                if(results[i]['formatted_address'].length > address.length) {
                                  address = results[i]['formatted_address'];
                                }

                                if(results[i]['types'].contains('sublocality')) {
                                  longName = results[i]['address_components'][0]['long_name'].replaceAll(new RegExp(r' '), '').toLowerCase();
                                  shortName = results[i]['address_components'][0]['short_name'].replaceAll(new RegExp(r' '), '').toLowerCase();
                                }
                              }

                              // _addressController.text = address;
                              var selectedZone = zones.where((zone) {
                                return zone['name'].replaceAll(new RegExp(r' '), '').toLowerCase() == longName || zone['name'].replaceAll(new RegExp(r' '), '').toLowerCase() == shortName;
                              }).toList();
                      ...
                      child: Container(
                        ...
                        child: Center(
                          child: Text(allTranslations.text('done').toUpperCase(), style: TextStyle(fontSize: 16,fontWeight: FontWeight.bold)),
                        ),
                      ),
                    )
                  ],
