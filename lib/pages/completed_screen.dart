...
class _CompletedScreenState extends State<CompletedScreen> {
  ...
  Future _getCompletedOrders() async {
    _sharedPreferences = await _prefs;
		String authToken = AuthUtils.getToken(_sharedPreferences);

    return NetworkUtils.fetch(authToken, '/api/Orders/complete');
  }
                ...
                if(snapshot.hasError) {
                  ...
                  if(_orders == 'NetworkError') {
                    ...
                    return RefreshIndicator(
                      onRefresh: _getCompletedOrders,
                      child: _orders != null && !_orders.isEmpty ? ListView.builder(
                        itemCount: _orders.length,
                        itemBuilder: (context, position) {
                          return Padding(
                            padding: const EdgeInsets.only(bottom: 30, right: 5),
                            child: AlmostDoneItem(order: _orders[position]),
                          );
                        }
                      ) : Center(
                        child: Column(
